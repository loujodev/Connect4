...
class BitBoard:
    def __init__(self):
        ...
        self.player_board = 0  # Bitboard for the current player
        self.opponent_board = 0  # Bitboard for the opponent
    ...
    def get_combined_board(self):
        ...
        """
        return self.player_board | self.opponent_board
    ...
    def print_board(self):
        ...
        for row in reversed(range(self.amount_rows)):
            ...
            for col in range(self.amount_columns):
                ...
                if (self.player_board >> bit_position) & 1:
                    row_str.append("X")
                elif (self.opponent_board >> bit_position) & 1:
                    row_str.append("O")
                else:
                    row_str.append(" ")
            print("  ┃  ".join(row_str))
    ...
    def is_empty(self):
        ...
        """
        return self.player_board == 0 and self.opponent_board == 0
    ...
    def play_move(self, column, symbol):
        ...
        for row in range(self.amount_rows):
            ...
            if not ((self.get_combined_board() >> bit_position) & 1):
                if symbol == "X":
                    self.player_board |= 1 << bit_position
                else:
                    self.opponent_board |= 1 << bit_position
                break

    def check_winner(self, bitboard):
        """
        Checks if there is a winner on the given bitboard.
    ...
    def undo_move(self, column, symbol):
        ...
        for row in range(self.amount_rows):
            bit_position = row * self.amount_columns + column
            if symbol == "X" and (self.player_board >> bit_position) & 1:
                self.player_board &= ~(1 << bit_position)
                break
            elif symbol == "O" and (self.opponent_board >> bit_position) & 1:
                self.opponent_board &= ~(1 << bit_position)
                break
    ...
    def get_winning_move(self, symbol, available_moves):
        ...
        for move in available_moves:
            ...
            if self.check_winner(self.player_board if symbol == "X" else self.opponent_board):
