...
from Code.game_logic.constants import DISTANCE_TO_BORDER, SCORE_TWO, SCORE_THREE, SCORE_WIN, \
    ...
    EMPTY, SCORE_BLOCK_OPPONENT_THREE, SEARCH_DEPTH, AMOUNT_ROWS, AMOUNT_COLUMNS
...
class BitboardMiniMaxAgent(Player):
    ...
    def choose_move(self, board):
        ...
        print(f"Verfügbare Züge: {available_moves}")
        ...
        return self.minimax(bitboard, SEARCH_DEPTH, True)[0]
    ...
    def convert_to_bitboard(self, game_board):
        ...
        """
        bitboard = BitBoard()
        bitboard.player_board = bitboard.to_bitboards(self.symbol, game_board.board)
        bitboard.opponent_board = bitboard.to_bitboards(self.opponent_symbol, game_board.board)
        return bitboard
    ...
    def evaluate_position(self, board):
        ...
        for col in CENTRAL_COLS:
            if (board.player_board >> ((AMOUNT_ROWS - 1) * AMOUNT_COLUMNS + col)) & 1:
                score += SCORE_CENTRAL

        # Evaluate horizontal, vertical, and diagonal sections
        score += self.evaluate_sections(board.player_board, board.opponent_board)
        print(f"Bewertung des aktuellen Zustands: {score}")
    ...
    def evaluate_sections(self, player_board, opponent_board):
        ...
        for row in range(AMOUNT_ROWS):
            for col in range(AMOUNT_COLUMNS - DISTANCE_TO_BORDER):
                ...
                for i in range(SECTION_LENGTH):
                    section_mask |= 1 << (row * AMOUNT_COLUMNS + col + i)
        ...
        for col in range(AMOUNT_COLUMNS):
            for row in range(AMOUNT_ROWS - DISTANCE_TO_BORDER):
                section_mask = 0
                for i in range(SECTION_LENGTH):
                    section_mask |= 1 << ((row + i) * AMOUNT_COLUMNS + col)
                score += self.evaluate_section(player_board, opponent_board, section_mask)
        ...
        for row in range(AMOUNT_ROWS - DISTANCE_TO_BORDER):
            for col in range(AMOUNT_COLUMNS - DISTANCE_TO_BORDER):
                section_mask = 0
                for i in range(SECTION_LENGTH):
                    section_mask |= 1 << ((row + i) * AMOUNT_COLUMNS + col + i)
                score += self.evaluate_section(player_board, opponent_board, section_mask)
    ...
    def evaluate_section(self, player_board, opponent_board, section_mask):
        ...
        if opponent_count == SECTION_LENGTH:
            score -= SCORE_WIN  # Opponent has won
        elif opponent_count == SECTION_LENGTH - 1 and empty_count == 1:
            score -= SCORE_BLOCK_OPPONENT_WIN  # Opponent has a potential winning move
        elif opponent_count == SECTION_LENGTH - 2 and empty_count == 2:
            score -= SCORE_BLOCK_OPPONENT_THREE  # Opponent has a potential three in a row
    ...
    def minimax(self, board, depth, maximizing, alpha=-float('inf'), beta=float('inf')):
        if depth == 0 or self.is_terminal(board):
            if self.is_terminal(board):
                if board.check_winner(self.symbol):
                    print("Gewinnzustand erkannt für den Spieler.")
                    return None, float('inf')
                elif board.check_winner(self.opponent_symbol):
                    print("Gewinnzustand erkannt für den Gegner.")
                    return None, -float('inf')
            eval_score = self.evaluate_position(board)
            print(f"Bewertung bei Tiefe {depth}: {eval_score}")
            return None, eval_score

        best_move = board.get_available_moves()[0]  # Standardmäßig der erste verfügbare Zug
        ...
        if maximizing:
            ...
            for move in board.get_available_moves():
                ...
                print(f"Maximierender Zug: {move}, Bewertung: {score}")
                if score > value:
                    value = score
                    best_move = move
                ...
                if alpha >= beta:
                    print("Alpha-Beta-Schnittstelle aktiviert.")
                    break
            ...
            for move in board.get_available_moves():
                ...
                print(f"Minimierender Zug: {move}, Bewertung: {score}")
                if score < value:
                    value = score
                    best_move = move
